...
function groupSchedulesByNode(schedules: InstanceBackupSchedule[]): Map<string, InstanceBackupSchedule[]> {
  const grouped = new Map<string, InstanceBackupSchedule[]>();
  
  for (const schedule of schedules) {
    const nodeName = schedule.nodeName || "Local";
    if (!grouped.has(nodeName)) {
      grouped.set(nodeName, []);
    }
    grouped.get(nodeName)!.push(schedule);
  }
...
export function SchedulesView() {
  ...
  // Group schedules by node
  const groupedSchedules = groupSchedulesByNode(schedules);
  const nodeNames = Array.from(groupedSchedules.keys()).sort();
      ...
      <div className="mb-8">
        ...
        {nodeNames.length > 1 && (
          <p className="mt-1 text-sm text-blue-600">
            Mesh mode: viewing {nodeNames.length} node(s)
          </p>
        )}
      </div>
      ...
      {schedules.length === 0 ? (
        ...
        <div className="space-y-8">
          {nodeNames.map((nodeName) => {
            const nodeSchedules = groupedSchedules.get(nodeName)!;
            return (
              <div key={nodeName}>
                {/* Node header (only show if multiple nodes) */}
                {nodeNames.length > 1 && (
                  <div className="mb-4 border-l-4 border-blue-500 pl-4">
                    ...
                    <p className="text-sm text-gray-600">
                      {nodeSchedules.length} backup schedule(s)
                    </p>
                  </div>
                )}
